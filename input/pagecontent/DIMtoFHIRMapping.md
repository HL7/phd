...
All Observation objects in ACOM contain one attribute that specifies an IEEE 11073-10101 nomenclature code expressing what the Observation is. In ACOM, this attribute is called "Type". For example, a body temperature, body mass, medication dosage, or non-invasive blood pressure. This ACOM Type attribute is mapped to the FHIR `Observation.code` element. In compound observations, the decoder must look at the component-type attributes as well, for example the systolic, diastolic, and MAP values of the non-invasive blood pressure. These component-type codes are mapped to `Observation.component.code` elements representing each component.
...
For relative (tick-counter) PHDs, the PHG anchors the measurement timestamps to its UTC timeline using the connection's Coincident Timestamp Observation. The converted `effective[x]` SHALL retain fractional-second precision consistent with the PHD's tick resolution, so that time differences between observations are preserved.
