...
###### Device Alerts
Device alerts are represented as separate [PhdDeviceAlert](StructureDefinition-PhdDeviceAlert.html) resources that reference the associated measurement observation(s) via the derivedFrom element. This approach provides a clear separation between measurement data and alert conditions generated by personal health devices.

A PhdDeviceAlert resource includes:

The alert type code (in `type`), which represents the MDC alert event code from the IEEE 11073 nomenclature.

The alert condition and threshold limits (in `condition.limit`), which define the range of values that trigger the alert.

The alert notification text (in `label`), which may contain the message displayed when the alert threshold is crossed.

The alert operational state and related operational text (in `signal` elements).

Reference(s) to the source measurement observation(s) that triggered the alert (in `derivedFrom`). These may include PhdNumericObservation, PhdCompoundNumericObservation, PhdCompoundObservation, or PhdBaseObservation.

This structure allows alerts to be modeled independently from measurement observations while maintaining a logical linkage between the alert condition and the underlying measurement data.
