...
Device alerts are typically generated by the Personal Health Gateway (PHG) rather than the personal health device itself. When the PHG detects an observation with a status code of `msmt-value-exceeded-boundaries`, it generates a corresponding DeviceAlert resource that references the source observation. This allows consumers to query for and manage alerts independently from measurement data.
